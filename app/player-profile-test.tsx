...
const PlayerProfileScreen = () => {
        ...
        <View key={match.id} className="bg-white rounded-2xl p-4 mb-4 shadow-sm border border-gray-100">
            ...
            <View className="space-y-2 mb-3">
                <View className="flex-row justify-between items-center">
                    <Text className="text-green-600 font-medium">{match.team1}</Text>
            ...
            <View className="flex-row justify-around border-t border-gray-200 pt-3">
                <TouchableOpacity className="flex-1 items-center">
                    <Text className="text-green-600 font-medium">INSIGHTS</Text>
                </TouchableOpacity>
                <TouchableOpacity className="flex-1 items-center">
                    <Text className="text-green-600 font-medium">TABLE</Text>
                </TouchableOpacity>
                <TouchableOpacity className="flex-1 items-center">
                    <Text className="text-green-600 font-medium">LEADERBOARD</Text>
                </TouchableOpacity>
            </View>
        </View>
    )
    ...
    return (
        <SafeAreaView className="flex-1 bg-gray-900">
            ...
            {/* Profile Section */}
            <View className="bg-gray-900 items-center pb-6">
                <Image
                    source={{
                ...
                />
                <Text className="text-2xl font-bold text-white mb-1">Saleem Khan</Text>
                <Text className="text-gray-300 mb-4">(Scorer - Karachi)</Text>
            ...
            {/* Rating and Price */}
            <View className="bg-gray-900 px-4 pb-4">
                <View className="flex-row justify-between items-center">
                    <Text className="text-white text-lg">Rs.1600/day, 800/ma...</Text>
                    <View className="flex-row items-center">
                        <Text className="text-white text-lg font-bold mr-2">5.0</Text>
                        <View className="flex-row">
                            {[1, 2, 3, 4, 5].map((star) => (
                                <Ionicons key={star} name="star" size={16} color="#fbbf24" />
                            ))}
                        </View>
                        <Text className="text-gray-300 ml-2">(4)</Text>
                    </View>
                </View>
            </View>
            ...
            <View className="bg-white">
                <ScrollView horizontal showsHorizontalScrollIndicator={false} className="px-4 py-3">
                    {tabs.map((tab) => (
                        <TouchableOpacity
                            key={tab}
                            className={`px-4 py-2 mr-4 ${activeTab === tab ? "border-b-2 border-yellow-500" : ""}`}
